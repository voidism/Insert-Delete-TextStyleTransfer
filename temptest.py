...
class IndexedDataset(Dataset):
    ...
    def __init__(self, path, max_len=256):
        ...
        self.lines = []
        self.sizes = []
        self.tokens_list_a, self.sizes_a = self.read_data(path+".cor.idx")
        self.tokens_list_b, self.sizes_b = self.read_data(path+".flt.idx")
    ...
    def decode_step_batched(self, raw_ids, ins, prd):
        ret_batch = []
        for i in range(len(raw_ids)):
            ret_batch.append(self.decode_step(raw_ids[i].tolist(), ins[i].tolist(), prd[i].tolist()))
...
if __name__ == "__main__":
    ...
    if args.do_train:
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=args.train_batch_size, collate_fn=IndexedDataset.collater)
