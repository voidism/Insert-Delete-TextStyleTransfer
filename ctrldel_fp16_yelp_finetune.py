...
from ctrldel_yelp_dataset import IndexedDataset
...
if __name__ == "__main__":
    ...
    roberta.load_state_dict(torch.load(os.path.join(args.from_dir, 'bert_model.bin')))
    # if args.delete:
    ...
    # init CTRL code
    roberta.roberta.embeddings.word_embeddings.weight[50261,:].data = roberta.roberta.embeddings.word_embeddings.weight[0,:].data
    roberta.roberta.embeddings.word_embeddings.weight[50262,:].data = roberta.roberta.embeddings.word_embeddings.weight[0,:].data
    ...
    if args.do_train:
        ...
        test_dataset = IndexedDataset(args.test_corpus)
        ...
        test_dataloader = DataLoader(test_dataset, batch_size=args.train_batch_size, collate_fn=IndexedDataset.collater)
    ...
    if args.do_train:
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                input_ids, predict_label, insert_label, delete_label, input_mask = batch
                ...
                delete_score = delete_net(seq_output)
                
                loss = loss_fct(prediction_scores.view(-1, prediction_scores.shape[-1]), predict_label.view(-1))
                insert_loss = loss_fct(insert_score.view(-1, insert_score.shape[-1]), insert_label.view(-1))
                delete_loss = loss_fct(delete_score.view(-1, delete_score.shape[-1]), delete_label.view(-1))

                now = time.time()
                rem = ((now - start) * ((total_steps - step) / step)) if step != 0 else - 1
                now_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
                text = "{time} [{epoch}/{total_epoch}] [{idx}/{total_idx}] loss: {loss:.4f} ppl: {ppl:.4f} insert: {insert:.4f} delete: {delete:.4f} ETA: {eta:.1f}".format(epoch=epoch,
                total_epoch=int(args.num_train_epochs), idx=step, total_idx=total_steps, loss=float(loss), ppl=math.exp(float(loss)), insert=float(insert_loss), delete=float(delete_loss), eta=rem, time=now_time)
                if use_wandb:
                    wandb.log({'loss': float(loss), 'ppl': math.exp(float(loss)), 'insert': float(insert_loss), 'delete': float(delete_loss)})
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    if args.fp16:
                        ...
                        torch.nn.utils.clip_grad_norm_(delete_net.parameters(), args.max_grad_norm)
                    optimizer.step()
                    scheduler.step()  # Update learning rate schedule
                    roberta.zero_grad()
                    insert_net.zero_grad()
                    delete_net.zero_grad()
                    global_step += 1
                ...
                if (step + 1) % 10000 == 0:
                    ...
                    output_model_file = os.path.join(args.output_dir, "delete_model.bin")
                    if args.do_train:
                        torch.save(delete_net.state_dict(), output_model_file)
                step += 1
            # Testing !
            print("[Tesing Epoch %d]"%epoch)
            roberta.eval()
            insert_net.eval()
            delete_net.eval()
            dev_loss = 0.0
            ...
            for step, batch in enumerate(test_dataloader):
                ...
                now = time.time()
                rem = ((now - start) * ((len(test_dataloader) - step) / step)) if step != 0 else - 1
                now_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
                text = "{time} [{epoch}/{total_epoch}] [{idx}/{total_idx}] loss: {loss:.4f} ppl: {ppl:.4f} insert: {insert:.4f} delete: {delete:.4f} ETA: {eta:.1f}".format(epoch=epoch,
                total_epoch=int(args.num_train_epochs), idx=step, total_idx=len(test_dataloader), loss=float(loss), ppl=math.exp(float(loss)), insert=float(insert_loss), delete=float(delete_loss), eta=rem, time=now_time)
                dev_loss += float(loss)
                dev_ppl += math.exp(float(loss))
                dev_insert += float(insert_loss)
                dev_delete += float(delete_loss)
                total_num += 1.0
            ...
            if use_wandb:
                wandb.log({'dev_loss': dev_loss/total_num, 'dev_ppl': dev_ppl/total_num, 'dev_insert': dev_insert/total_num, 'dev_delete': dev_delete/total_num}, step=global_step)
            # Save a trained model
            logger.info("** ** * Saving fine - tuned model ** ** * \n")
            model_to_save = roberta.module if hasattr(roberta, 'module') else roberta  # Only save the model it-self
            output_model_file = os.path.join(args.output_dir, "bert_model.bin.%04d"%epoch)
            if args.do_train:
                torch.save(roberta.state_dict(), output_model_file)
            output_model_file = os.path.join(args.output_dir, "insert_model.bin.%04d"%epoch)
            if args.do_train:
                torch.save(insert_net.state_dict(), output_model_file)
            output_model_file = os.path.join(args.output_dir, "delete_model.bin.%04d"%epoch)
            if args.do_train:
                torch.save(delete_net.state_dict(), output_model_file)
