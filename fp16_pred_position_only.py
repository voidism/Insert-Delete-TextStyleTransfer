    ...
    if args.do_train:
        ...
        for epoch, idxfile in enumerate(scp):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                text = "{time} [{epoch}/{total_epoch}] [{idx}/{total_idx}] insert: {insert:.4f} ETA: {eta:.1f}".format(epoch=epoch,
                total_epoch=int(args.num_train_epochs), idx=step, total_idx=total_steps, insert=float(insert_loss), eta=rem, time=now_time)
                if use_wandb:
                    wandb.log({'insert': float(insert_loss)})
                loss = insert_loss
                # else:
                #     now = time.time()
                #     rem = ((now - start) * ((total_steps - step) / step)) if step != 0 else - 1
