    ...
    if args.do_train:
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                text = "{time} [{epoch}/{total_epoch}] [{idx}/{total_idx}] loss: {loss:.4f} ppl: {ppl:.4f} insert: {insert:.4f} ETA: {eta:.1f}".format(epoch=epoch,
            ...
            for step, batch in enumerate(test_dataloader):
                ...
                now_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
                text = "{time} [{epoch}/{total_epoch}] [{idx}/{total_idx}] loss: {loss:.4f} ppl: {ppl:.4f} insert: {insert:.4f} ETA: {eta:.1f}".format(epoch=epoch,
                total_epoch=int(args.num_train_epochs), idx=step, total_idx=len(test_dataloader), loss=float(loss), ppl=math.exp(float(loss)), insert=float(insert_loss), eta=rem, time=now_time)
                dev_loss += float(loss)
