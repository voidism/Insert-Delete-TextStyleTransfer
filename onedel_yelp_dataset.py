...
class IndexedDataset(Dataset):
    ...
    def __init__(self, path, max_len=256):
        ...
        self.lines = []
        self.sizes = []
        tokens_list_ins_neg_a, sizes_ins_neg_a = self.read_data(path + ".0.flt.idx", attr=0)
        tokens_list_ins_pos_a, sizes_ins_pos_a = self.read_data(path + ".1.flt.idx", attr=1)
        tokens_list_del_neg_a, sizes_del_neg_a = self.read_data(path + ".0.cor.idx", attr=1)
        tokens_list_del_pos_a, sizes_del_pos_a = self.read_data(path + ".1.cor.idx", attr=0)
        self.tokens_list_ins_a = tokens_list_ins_neg_a + tokens_list_ins_pos_a
        self.tokens_list_del_a = tokens_list_del_neg_a + tokens_list_del_pos_a
        self.sizes_ins_a = sizes_ins_neg_a + sizes_ins_pos_a
        self.sizes_del_a = sizes_del_neg_a + sizes_del_pos_a
        tokens_list_ins_neg_b, sizes_ins_neg_b = self.read_data(path + ".0.cor.idx", attr=0)
        tokens_list_ins_pos_b, sizes_ins_pos_b = self.read_data(path + ".1.cor.idx", attr=1)
        tokens_list_del_neg_b, sizes_del_neg_b = self.read_data(path + ".0.flt.idx", attr=1)
        tokens_list_del_pos_b, sizes_del_pos_b = self.read_data(path + ".1.flt.idx", attr=0)
        self.tokens_list_ins_b = tokens_list_ins_neg_b + tokens_list_ins_pos_b
        self.tokens_list_del_b = tokens_list_del_neg_b + tokens_list_del_pos_b
        self.sizes_ins_b = sizes_ins_neg_b + sizes_ins_pos_b
        self.sizes_del_b = sizes_del_neg_b + sizes_del_pos_b
        self.size = len(self.tokens_list_ins_a) + len(self.tokens_list_del_a)
        assert self.size == len(self.tokens_list_ins_b) + len(self.tokens_list_del_b)
    ...
    def aggregate_samples(self):
        self.samples_list = []
        for i in tqdm.trange(len(self.tokens_list_del_a)):
            self.samples_list += self.generate_sample(i, dele=0)
            self.samples_list += self.generate_sample(i, dele=1)
        self.size = len(self.samples_list)


    def generate_sample(self, idx, dele=0):
        a, b = self.get_sent_pair(idx, dele=dele)[:]
        # ops = edit_ops(a, b)
        actions = []
        while True:
            ops = edit_ops(a, b)
            ...
            if len(ops) == 0:
                if dele == 0:
                    actions.append((input_ids, insert_label, [-1]*len(insert_label), predict_label))
                else:
                    actions.append((input_ids, [-1]*len(insert_label), insert_label, predict_label))
                break
            random.shuffle(ops)
            remain_ops = []
            for op in ops:
    ...
    def wrapped_decode_step(self, raw_ids, ins, dele, prd):
        if (torch.tensor(dele) == -1).all():
            return self.decode_step(raw_ids, ins, prd)
        elif (torch.tensor(ins) == -1).all():
            return self.decode_step(raw_ids, dele, prd)
        else:
            raise ValueError("Invalid input: ins or dele should be all -1.")
    ...
    def unittest(self, idx):
        actions = self.generate_sample(idx, dele=0)
        a, b = self.get_sent_pair(idx, dele=0)
        output = a[:]
        ...
        for i in range(len(actions)):
            ...
            output = self.wrapped_decode_step(*actions[i])
            # print("Action {} (after ): {}".format(i, self.tokenizer.decode(output)))
        # print("After all Actions: {}".format(self.tokenizer.decode(b)))
        assert b == output
        actions = self.generate_sample(idx, dele=1)
        a, b = self.get_sent_pair(idx, dele=1)
        output = a[:]
        # print("Before all Action: {}".format(self.tokenizer.decode(a)))
        for i in range(len(actions)):
            assert output == actions[i][0] or print(output, actions[i][0])
    ...
    @lru_cache(maxsize=8)
    def get_sent_pair(self, i, dele=0):
        self.check_index(i)
        if dele == 0:
            return self.tokens_list_ins_a[i][:self.max_len-1] + [2], self.tokens_list_ins_b[i][:self.max_len-1] + [2]
        else:
            return self.tokens_list_del_a[i][:self.max_len-1] + [2], self.tokens_list_del_b[i][:self.max_len-1] + [2]


    @lru_cache(maxsize=8)
    def __getitem__(self, i):
        self.check_index(i)
        term = self.samples_list[i] # input_ids, insert_label, delete_label, predict_label
        return torch.tensor(term[0]), torch.tensor(term[3]), torch.tensor(term[1]), torch.tensor(term[2]), torch.ones(len(term[0]))
