...
class IndexedDataset(Dataset):
    ...
    def __init__(self, path, max_len=512):
        ...
        self.lines = []
        self.sizes = []
        self.tokens_list_a, self.sizes_a = self.read_data(path+".flt.idx")
        self.tokens_list_b, self.sizes_b = self.read_data(path+".cor.idx")
    ...
    def read_data(self, path):
        ...
        with open(path, 'r', encoding='utf-8') as f:
            for line in f:
                line = line.strip('\n')
                lines.append(line)
                tokens = [int(x) for x in line.split(' ')] if len(line) > 0 else []
    ...
    def decode_step_batched(self, raw_ids, ins, prd, dele, mask):
        ret_batch = []
        lengths = mask.sum(-1).int().tolist()
        lens = []
        for i, length in zip(range(len(raw_ids)), lengths):
            ret_batch.append(self.decode_step(raw_ids[i][:length].tolist(), ins[i][:length].tolist(), dele[i][:length].tolist(), prd[i][:length].tolist()))
    ...
    def decode_step(self, raw_ids, ins, dele, prd):
        ret = []
        for i in range(len(ins)):
            if ins[i] == 1:
                # print("insertion:", self.tokenizer.decode([raw_ids[i]]), "is inserted")
                ret.append(prd[i])
            if dele[i] == 1:
    ...
    def __getitem__(self, i):
        self.check_index(i)
        sent = [0] + self.tokens_list_a[i][:self.max_len-2] + [2]
...
if __name__ == "__main__":
    ...
    insert_net = nn.Linear(768, 2) if not args.block else nn.Sequential(nn.Linear(768, 2048), nn.Dropout(0.2), nn.LeakyReLU(inplace=True), nn.Linear(2048, 2))
    delete_net = nn.Linear(768, 2) if not args.block else nn.Sequential(nn.Linear(768, 2048), nn.Dropout(0.2), nn.LeakyReLU(inplace=True), nn.Linear(2048, 2))
    ...
    fw = open(args.output_dir, 'w')

    def write_file(tensor):
        for i in range(len(tensor)):
            fw.write(tokenizer.decode([x for x in tensor[i].tolist() if x not in [0, 1, 2]]) + '\n')
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        with torch.no_grad():
            for iteration in range(5):
                ...
                dele = (delete_score[..., 1] > args.threshold)
                prd = prediction_scores.argmax(-1)
                input_ids, input_mask = train_dataset.decode_step_batched(input_ids, ins, prd, dele, input_mask)
