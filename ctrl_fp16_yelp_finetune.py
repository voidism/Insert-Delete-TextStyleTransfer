...
from ctrl_yelp_dataset import IndexedDataset
...
if __name__ == "__main__":
    ...
    insert_net = nn.Linear(768, 3) if not args.delete else nn.Linear(768, 3)
    ...
    model = tempmodel(roberta, insert_net)
    ...
    if args.do_train:
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                total_epoch=int(args.num_train_epochs), idx=step, total_idx=total_steps, loss=float(loss), ppl=math.exp(float(loss)), insert=float(insert_loss), eta=rem, time=now_time)
                if use_wandb:
            ...
            if use_wandb:
                wandb.log({'dev_loss': dev_loss/total_num, 'dev_ppl': dev_ppl/total_num, 'dev_insert': dev_insert/total_num}, step=global_step)
            # Save a trained model
            logger.info("** ** * Saving fine - tuned model ** ** * \n")
            model_to_save = roberta.module if hasattr(roberta, 'module') else roberta  # Only save the model it-self
